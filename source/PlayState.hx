...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['imagine', 0.4], //From 20% to 39%
		['touch grass', 0.5], //From 40% to 49%
		['Bruh', 0.6], //From 50% to 59%
		['ehhhh', 0.69], //From 60% to 68%
		['Pogr', 0.7], //69%
		['still kinda good', 0.8], //From 70% to 79%
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['rearranger', 'the_end', 'cal', 'rude',
				'week5_nomiss', 'week6_nomiss', 'week7_nomiss', 'ur_bad',
				'ur_good', 'hype', 'two_keys', 'toastie', 'debugger', 'the_end', 'rude']);
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				{
					case 'rearranger' | 'the_end' | 'cal' | 'rude' | 'week5_nomiss' | 'week6_nomiss' | 'week7_nomiss' | 'the_end' | 'rude':
						if(isStoryMode && campaignMisses + songMisses < 1000 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1000 && !changedDifficulty && !usedPractice)
						{
							var weekName:String = WeekData.getWeekFileName();
							switch(weekName) //I know this is a lot of duplicated code, but it's easier readable and you can add weeks with different names than the achievement tag
							{
								case 'rubix':
									if(achievementName == 'rearranger') unlock = true;
								case 'rubix-v-tord':
									if(achievementName == 'the_end') unlock = true;
								case 'cal':
									if(achievementName == 'cal') unlock = true;
								case 'peep':
									if(achievementName == 'rude') unlock = true;
								case 'week5':
